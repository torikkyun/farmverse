...
import NFTCard from "../items/nft-card";
import DungCard from "../items/dung-card";
import { NFTItem, DungItem } from "./types";
import { useFarmDetail } from "./useFarmDetail";

const API_URL = process.env.NEXT_PUBLIC_API_URL;

type CareSchedule = { month: number; activities: string[] };
...
function NoData({
  ...
  return (
    <div className="text-center py-24">
      ...
      <p className="text-gray-500 text-xl max-w-md mx-auto">{desc}</p>
    </div>
  );
}
  ...
  if (schedules.length === 0)
    return (
      <NoData
        ...
        desc={`Hiện tại chưa có lịch chăm sóc nào được thiết lập cho trang trại này.`}
      />
    );
  return (
    <div className="bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden">
      ...
      <div className="p-12 relative">
        ...
        {schedules.map((schedule, idx) => (
          <div key={idx} className="relative flex items-start mb-16 group">
...
function ItemsTab({
  ...
  selectedItems: { id: string; quantity: number }[];
  handleSelect: (id: string) => void;
  CardComponent: typeof NFTCard | typeof DungCard;
  emptyTitle: string;
  emptyDesc: string;
}) {
  return (
          ...
          {items.map((item, index) => (
            ...
            >
              {CardComponent === NFTCard ? (
                <NFTCard
                  item={item as NFTItem}
                  selected={selectedItems.some((sel) => sel.id === item.id)}
                  onSelect={handleSelect}
                />
              ) : (
                <DungCard
                  dungs={item as DungItem}
                  selected={selectedItems.some((sel) => sel.id === item.id)}
                  onSelect={handleSelect}
                />
              )}
      ...
      <main className="flex-1 bg-gray-50 min-h-screen">
        {activeTab === 2 ? (
          <div className="max-w-6xl mx-auto px-6 py-8">
            <ScheduleTab schedules={schedules} farmName={farm?.name} />
          </div>
        ) : activeTab === 0 ? (
          <ItemsTab
            items={items}
            ...
            CardComponent={NFTCard}
            emptyTitle="Chưa có cây trồng"
            emptyDesc="Hiện tại chưa có cây trồng nào có sẵn."
          />
        ) : (
          <ItemsTab
            items={dungs}
            selectedItems={selectedItems}
            handleSelect={handleSelect}
            CardComponent={DungCard}
            emptyTitle="Chưa có phân bón"
            emptyDesc="Hiện tại chưa có phân bón nào có sẵn."
          />
        )}
      </main>
    </>
